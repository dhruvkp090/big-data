...
public class AssessedExercise {
	...
	public static void main(String[] args) {
		...
		if (newsFile == null)
			newsFile = "data/big.json"; // default is a sample of 5000 news
																				// articles
...
//		// ----------------------------------------------------------------
		...
		for (Query q : queryList) {
			...
			List<RankedResult> sorted = rankedDocuments.collectAsList();
			rankedQueries.add(new DocumentRanking(q, sorted)); //Document ranking keeps only 10 reults, need to alter this
		}
		Dataset<DocumentRanking> out = spark.createDataset(rankedQueries, Encoders.bean(DocumentRanking.class)).map(new DocMapper(), Encoders.bean(DocumentRanking.class));
...
//		}
		...
		return out.collectAsList(); // replace this with the the list of DocumentRanking output by your topology
	}
